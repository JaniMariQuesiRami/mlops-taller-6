...
variable "shared_volume_name" {
  type        = string
  description = "Nombre del volumen compartido con ciencia de datos."
}
...
resource "docker_image" "data_eng" {
...
resource "docker_container" "data_eng" {
  name  = var.container_name
  image = docker_image.data_eng.image_id
...
output "image_id" {
  value       = docker_image.data_eng.image_id
  description = "ID único de la imagen de ingeniería de datos construida por Terraform."
}

output "container_id" {
  value       = docker_container.data_eng.id
  description = "ID del contenedor de ingeniería de datos desplegado."
}
